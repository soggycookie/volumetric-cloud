...
public class WeatherSystem : MonoBehaviour
{
    ...
    public float weatherChangeTime = 20;
    ...
    private void Update()
    {
        ...
        if (elapsedTime >= weatherChangeTime)
        {
            elapsedTime = 0;
            Blend();
        }
    ...
    public void Generate(WeatherData data)
    {
        ...
        noiseCS.SetVector("_Randomness",
            new Vector3(Random.Range(0, 10000), Random.Range(0, 1000), Random.value * .5f));
...
public struct WeatherData
{
    ...
    public WeatherData(RenderTexture map, Vector3 resolution, int density, int seed, float fallOffCoverage,
        float lowAltitudeCloudBias, float mediumAltitudeCloudBias, float highAltitudeCloudBias, float smallCloudBias)
    {
        this.map = map;
